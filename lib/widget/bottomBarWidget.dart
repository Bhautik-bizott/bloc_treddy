...
class CircularBottomNavigation extends StatefulWidget {
  ...
  const CircularBottomNavigation(
    this.tabItems, {
    super.key,
    this.selectedPos = 0,
    this.barHeight = 60,
  ...
  }) /*: backgroundBoxShadow = backgroundBoxShadow ?? [const BoxShadow(color: Colors.white, blurRadius: 10.0)],
...
class _CircularBottomNavigationState extends State<CircularBottomNavigation> with TickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    boxes.asMap().forEach((int pos, Rect r) {
      ...
      children.add(
        Positioned(
          left: r.center.dx - (widget.iconsSize / 2.0),
          top: pos == selectedPos ?r.center.dy - (widget.iconsSize / 2) - (_itemsSelectedState[pos] * ((widget.barHeight / 2) + widget.circleStrokeWidth)):r.center.dy - (widget.iconsSize / 2) - (_itemsSelectedState[pos] * ((widget.barHeight / 2) + widget.circleStrokeWidth)) - 10,
          child: Image.asset(
      ...
      // print(r.top + (widget.circleSize / 2) - (widget.circleStrokeWidth * 2) + ((1.0 - _itemsSelectedState[pos]) * textHeight));
      children.add(
        Positioned(
          left: r.left,
          top: 60,
